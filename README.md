## Hi, I'm Lihan 👋

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&pause=1000&color=2D9CDB&center=true&vCenter=true&width=700&lines=Full-Stack+Web+Developer;AI+%2F+ML+Engineer;Building+LLM+agents%2C+ML+pipelines+%26+secure+web+apps;Always+learning%2C+always+shipping" alt="Typing SVG" />

[![Portfolio](https://img.shields.io/badge/Portfolio-000?style=for-the-badge&logo=About.me&logoColor=white)](https://tahsanlihan.vercel.app/) [![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/tahsan-ferdous-b791753aa/) [![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:tahsanferdous@gmail.com)

I'm a full-stack developer and AI/ML engineer who ships end-to-end: React/Next.js frontends, Node.js and Python backends, and the ML or LLM layer that makes a product actually smart. I care about measuring before building — ablations over assertions, calibrated risk scores over vibes — and about keeping user data and trust intact along the way.

## What I focus on
- **AI / ML engineering** — LLM-orchestrated agents (multi-node workflows, tool use), applied ML (sensor-based classification, NLP benchmarking), and rigorous evaluation before shipping.
- **Full-stack web engineering** — React/Next.js on the frontend, Node.js/Express and Python/FastAPI on the backend, with Postgres, MongoDB, and Firebase as the data layer.
- **Security-first product thinking** — zero-knowledge proofs, scam/fraud triage, and privacy-preserving design baked in from the start, not bolted on after.

## Recent builds & hackathons
| Project | Built for | Focus |
| --- | --- | --- |
| [CI Sentinel](https://github.com/LihanCanCode/Ci-Sentinental) | Google All Things Agentic Hackathon | Autonomous Gemini + Google ADK agent that triages CI/CD failures |
| [CreditProof](https://github.com/LihanCanCode/CreditProof) | Brainwave 2026 — Midnight Track | Zero-knowledge financial reputation on the Midnight blockchain |
| [AI Front Desk](https://github.com/LihanCanCode/Ai_FrontDesk) | CALL-E: Your Code Is Calling | AI receptionist that fields real phone calls to fill service-business calendars |
| [MedLingo](https://github.com/LihanCanCode/MedLingo) | Next Byte Hacks V3 | Turns prescriptions/lab reports into plain-language, patient-friendly text |
| [CoShop](https://github.com/LihanCanCode/Co-shop) | OpenAI WebMCP Challenge | Storefront where a shopper and their AI agent browse and check out together |

## Tech stack
- **Languages:** ![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white) ![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black) ![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white) ![C++](https://img.shields.io/badge/C++-00599C?style=flat&logo=c%2b%2b&logoColor=white) ![C](https://img.shields.io/badge/C-A8B9CC?style=flat&logo=c&logoColor=black)
- **AI / ML:** ![Google Gemini](https://img.shields.io/badge/Gemini_API-8E75B2?style=flat&logo=google&logoColor=white) ![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white) ![Jupyter](https://img.shields.io/badge/Jupyter-F37626?style=flat&logo=jupyter&logoColor=white) ![NumPy](https://img.shields.io/badge/NumPy-013243?style=flat&logo=numpy&logoColor=white) ![pandas](https://img.shields.io/badge/pandas-150458?style=flat&logo=pandas&logoColor=white) Google ADK, applied ML classification, LLM evaluation/benchmarking
- **Frontend:** ![React](https://img.shields.io/badge/React-20232A?style=flat&logo=react&logoColor=61DAFB) ![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat&logo=nextdotjs&logoColor=white) ![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat&logo=tailwind-css&logoColor=white) ![Framer Motion](https://img.shields.io/badge/Framer_Motion-0055FF?style=flat&logo=framer&logoColor=white) ![Vite](https://img.shields.io/badge/Vite-646CFF?style=flat&logo=vite&logoColor=white)
- **Backend & APIs:** ![Node.js](https://img.shields.io/badge/Node.js-339933?style=flat&logo=node.js&logoColor=white) ![Express](https://img.shields.io/badge/Express-000000?style=flat&logo=express&logoColor=white) ![FastAPI](https://img.shields.io/badge/FastAPI-009688?style=flat&logo=fastapi&logoColor=white) ![Prisma](https://img.shields.io/badge/Prisma-2D3748?style=flat&logo=prisma&logoColor=white) Socket.IO
- **Data & storage:** ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white) ![MongoDB](https://img.shields.io/badge/MongoDB-47A248?style=flat&logo=mongodb&logoColor=white) ![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat&logo=firebase&logoColor=black) ![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat&logo=sqlite&logoColor=white) Supabase
- **Blockchain:** ![Midnight](https://img.shields.io/badge/Midnight-Compact-8A2BE2?style=flat&logo=blockchaindotcom&logoColor=white) Zero-knowledge proofs
- **DevOps & cloud:** ![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat&logo=github-actions&logoColor=white) ![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white) ![Vercel](https://img.shields.io/badge/Vercel-000000?style=flat&logo=vercel&logoColor=white) Google Cloud Run

## GitHub activity

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/LihanCanCode/LihanCanCode/output/github-contribution-grid-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/LihanCanCode/LihanCanCode/output/github-contribution-grid-snake.svg" />
    <img alt="GitHub contribution snake animation" src="https://raw.githubusercontent.com/LihanCanCode/LihanCanCode/output/github-contribution-grid-snake.svg" />
  </picture>
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=LihanCanCode&theme=tokyonight&hide_border=true" alt="GitHub streak stats" />
</p>

## Spotlight projects

### 🤖 AI & ML engineering
| Project | What it does |
| --- | --- |
| [VoltGuard](https://github.com/LihanCanCode/Volt-Guard) | Turns any phone into an AI fall-detection device with live caregiver alerts — no wearable hardware. |
| [Tripwire](https://github.com/LihanCanCode/Tripwire) | A five-node LLM workflow that triages scam messages (English/Bengali) with a calibrated risk score. |
| [BanglaPhonologyBench](https://github.com/LihanCanCode/BanglaPhonologyBench) | A phonology benchmark for Bangla LLMs, quantifying how badly subword tokenizers mangle an abugida script. |
| [Blind Spot](https://github.com/LihanCanCode/Blind-Spot) | AI tutor that tests real comprehension (vs. the "illusion of competence") using the Feynman technique. |
| [MedLingo](https://github.com/LihanCanCode/MedLingo) | Translates confusing prescriptions and lab reports into plain, patient-friendly language. |
| [CI Sentinel](https://github.com/LihanCanCode/Ci-Sentinental) | An autonomous Gemini + Google ADK agent that triages CI/CD pipeline failures. |
| [AI Front Desk](https://github.com/LihanCanCode/Ai_FrontDesk) | An AI receptionist that takes real phone calls to keep a service business's calendar full. |

### 🌐 Full-stack web platforms
| Project | What it does |
| --- | --- |
| [Thikana AI](https://github.com/LihanCanCode/ThikanaAi) | Bilingual, AI-powered rental & matchmaking marketplace for Bangladesh. |
| [Event Ekhane](https://github.com/LihanCanCode/EventEkhane) | Full event management platform: ticketing with QR validation, sponsors, and real-time messaging. |
| [DeshiTrip](https://github.com/LihanCanCode/DeshiTrip) | Bilingual travel companion for Bangladesh with curated spots and offline-resilient shared expenses. |
| [MoneyTracker](https://github.com/LihanCanCode/MoneyTracker) | Full-stack personal finance tracker: React dashboard, Express API, Firebase auth, Gemini automation. |
| [Nirapotta Mobile](https://github.com/LihanCanCode/Nirapotta_Mobile) | A mobile safety app that helps people protect themselves and respond in emergencies. |
| [CoShop](https://github.com/LihanCanCode/Co-shop) | A storefront where a shopper and their AI agent browse, compare, and check out in the same session. |

### 🔐 Security & blockchain
| Project | What it does |
| --- | --- |
| [CreditProof](https://github.com/LihanCanCode/CreditProof) | Zero-knowledge financial reputation built on the Midnight blockchain — proves a credential without revealing the data behind it. |

## Education
- Islamic University of Technology (IUT) — CSE
- Khulna Public College

## Let's build together
I'm always open to collaborations where AI/ML, full-stack engineering, and robust security meet. Drop a note and let's make something resilient.
